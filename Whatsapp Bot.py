...
def get(site):
    ...
    return driver
def wiki(driver):
    xpath="//span[@title='News']"
    driver.find_element_by_xpath('//*[@title="News"]').click()
...
if __name__=='__main__':
    ...
    driver=get(site)
    wiki(driver)
    search_engine = webdriver.Firefox()
    send_message("Hi Vishnu's Bot here",10,driver)
    state=1
    while state:
        state=mes(driver,search_engine)
    driver.quit()
    search_engine.quit()
